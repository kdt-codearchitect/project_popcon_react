...
const ProductComponentOnePlus = ({place, updateCheckedskuIdx}) => {
  ...
  const fetchMoreItems = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        skuTypeIdx !== 1000
          ? `${url}/sku/oneplus/type/${skuTypeIdx}/${page * 12}/${customerIdx}`
          : `${url}/sku/oneplus/${page * 12}/${customerIdx}`
      );
  ...
  const handleAddToWishlist = async (product) => {
    ...
    try {
      const response = await fetch(
        `${url}/wish/add/${Number.parseInt(customerIdx, 10)}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleRemove = async (product) => {
    ...
    try {
      const response = await axios.delete(
        `${url}/wish/delete/${customerIdx}/${product.skuIdx}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
...
export default ProductComponentOnePlus;
